...
class GMM:
	...
	def _prob_pixel_component(self, pixel, ci):
		...
		nt = np.asarray([t])
		# print(nt)
		expo = np.dot(inv, np.transpose(nt))
		expo = np.dot(nt, expo)
		# print(det)
		# print(expo)
...
class GCClient:
	...
	def assign_GMM_components(self):
		...
		for y in range(self.rows):
			for x in range(self.cols):
				pixel = self.img[y, x]
				self.components_index[y, x] = self.BGD_GMM.most_likely_pixel_component(pixel)[0] if (self._mask[y, x] \
					== self._GC_BGD or self._mask[y, x] == self._GC_PR_BGD) else self.FGD_GMM.most_likely_pixel_component(pixel)[0]
		...
		for y in range(self.rows):
			for x in range(self.cols):
				...
				if self._mask[y, x] == self._GC_PR_BGD or self._mask[y, x] == self._GC_PR_FGD:
					...
					fromSource = -np.log(self.BGD_GMM.prob_pixel_GMM(color))[0] *10
					toSink = -np.log(self.FGD_GMM.prob_pixel_GMM(color))[0]*10
					# print(fromSource)
		...
		for y in range(1, self.rows-1):
			for x in range(1, self.cols-1):
				...
				if a==b==3 or a==c==3 or a==d==3 or b==c==3 or b==d==3 or c==d==3:
					self._mask[x, y] = self._mask[x-1, y]
